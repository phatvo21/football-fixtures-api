...
describe('FixtureService', () => {
  ...
  beforeEach(async () => {
    ...
    fixtureService = new FixtureService(fixtureRepo);
  });
  ...
  it('should invoke the getAllFixture method from fixture repository when request to get the fixtures', async () => {
    await fixtureService.findAll({} as GetFixtureQueryDto);

    expect(fixtureRepo.getAllFixture).toHaveBeenCalled();
    expect(fixtureRepo.getAllFixture).toHaveBeenCalledTimes(1);
  });

  it('should invoke the getAllFixture method with the given params when received any query params from the request', async () => {
    await fixtureService.findAll({
      season: '',
      page: 3,
      size: 30,
      venue: '',
    ...
    expect(fixtureRepo.getAllFixture).toHaveBeenCalledWith({
      season: '',
      page: 3,
      size: 30,
      venue: '',
  ...
  it('should return all the fixture that are oder by latest to oldest fixture match dates and size equal 2, page equal 1', async () => {
    const result = await fixtureService.findAll({} as GetFixtureQueryDto);
    const { data } = result;
  ...
  it('should return all the enabled matches', async () => {
    const result = await fixtureService.findEnabledMatches({} as GetFixtureQueryDto);
  ...
  it('should invoke the findEnabledMatches method from fixture repository when request to get the matches', async () => {
    await fixtureService.findEnabledMatches({} as GetFixtureQueryDto);

    expect(fixtureRepo.getAllEnabledMatches).toHaveBeenCalled();
    expect(fixtureRepo.getAllEnabledMatches).toHaveBeenCalledTimes(1);
  });

  it('should invoke the getAllEnabledMatches method with the given params when received any query params from the request', async () => {
    await fixtureService.findEnabledMatches({
      season: '',
      venue: '',
      tournament: '',
      matchDate: '',
    ...
    expect(fixtureRepo.getAllEnabledMatches).toHaveBeenCalledWith({
      season: '',
      venue: '',
      tournament: '',
      matchDate: '',
